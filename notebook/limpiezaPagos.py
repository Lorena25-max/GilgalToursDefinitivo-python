...
def limpiar_datos_pago(data_frame_pago_sucio):
    ...
    data_frame_pago_limpio=data_frame_pago_limpio[data_frame_pago_limpio["monto"]>150000] """

    # Convertir con manejo de errores
    data_frame_pago_limpio["id_reserva"] = pd.to_numeric(
    data_frame_pago_limpio["id_reserva"], errors="coerce")

    data_frame_pago_limpio["id_pago"] = pd.to_numeric(
    data_frame_pago_limpio["id_pago"], errors="coerce")
    
    data_frame_pago_limpio["monto"] = pd.to_numeric(
    data_frame_pago_limpio["monto"], errors="coerce")
    
    # Eliminar filas inválidas en ambas columnas
    data_frame_pago_limpio = data_frame_pago_limpio.dropna(
    subset=["id_reserva", "monto", "id_pago"])

    #Rutina para evaluar fechas
    #Evaluar que una fecha si es una fecha
    data_frame_pago_limpio["fecha_pago"]=pd.to_datetime(data_frame_pago_limpio["fecha_pago"], errors="coerce")
